...
// The scoring engine: tracks the current run's notes/accuracy/misses and the loaded personal best,
// and formats them for display. Owned by a BattleSession; one instance per battle.
public class GameStats
{
    ...
    public void OutputCurrentNoteDebuggingData(bool isInGame)
    {
        ...
        if (isInGame)
            Log.Warn($"=========== Tick: {_stage?.realTimeTick ?? -1} | CurId: {_stage?.curTimeNodeOrder?.idx ?? -1} ===========");
        else
    ...
    public void CountMul(short oid, int result, float time)
    {
        switch (result)
        {
            ...
            case 3 or 4:
                ...
                break;
            default:
                Log.Warn($"Unknown result type of mul: {result}");
                break;
        }
    }
    ...
    public void Init()
    {
        ...
        var stats = StatsStore.GetStats(PlayingMusicHash);
        if (stats != null)
        {
